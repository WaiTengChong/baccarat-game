...
#### 模式選擇
- **⚡ 超快記憶體模式**  
 1. 後端直接在記憶體中建立牌靴並分配工作給多個 worker。  
 2. worker 以純記憶體方式完成所有手牌運算與連勝分析。  
 3. 不進行任何資料庫寫入或讀取。  
 4. 主執行緒彙整每個 worker 的統計結果後回傳前端。  
 5. 適合需要極速回饋的大量模擬或臨時測試。

- **💾 資料庫模式**  
 1. 啟動時建立暫存 SQLite 資料庫並寫入模擬資訊。  
 2. 依總局數自動選擇儲存策略：  
    - *標準模式*：儲存每局以及每手的完整資料。  
    - *大型模式 (Mega)*：僅儲存遊戲摘要與預先計算的連勝統計。  
 3. worker 回傳摘要資料，由主執行緒批次寫入資料庫。  
 4. 模擬完成後可透過 API 查詢詳細或摘要結果。  
 5. 適合需要保存紀錄以供後續分析的情境。

#### 遊戲模式
- **🎯 標準模式**  
 1. 每局遊戲開始前都重新洗牌並依設定的飛牌數丟棄牌。  
 2. 每局使用獨立的牌靴，在指定手數結束後重新洗牌。  
 3. 適合一般統計分析與常規模擬。

- **🔗 連貫模式**  
 1. 同一次運行中的所有遊戲共用一副連貫的牌靴。  
 2. 初始只洗牌並飛牌一次，之後牌靴在各遊戲間持續使用。  
 3. 牌靴用量不足時會重新洗牌，但會保留剩餘的牌。  
 4. 所有手牌結果合併後進行連勝統計。  
 5. 適合模擬真人賭場「不重洗連續發牌」的情境。
